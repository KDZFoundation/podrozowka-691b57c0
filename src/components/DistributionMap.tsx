...
const countryCoordinates: Record<string, { lat: number; lng: number }> = {
  "Polska": { lat: 52.0693, lng: 19.4803 },
  ...
  "Korea Południowa": { lat: 35.9078, lng: 127.7669 },
...
const DistributionMap = () => {
  ...
  const leafletColors = useMemo(() => {
    if (typeof window === "undefined") {
      return { primary: "hsl(0 72% 45%)", accent: "hsl(145 35% 32%)" };
    }
      ...
      if (!error && data) {
        ...
        const countryArray: CountryCount[] = Object.entries(countMap)
          .map(([country, count]) => {
            ...
            if (coords) {
              return { country, count, lat: coords.lat, lng: coords.lng };
            }
            return null;
          })
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Śledź na żywo, do których krajów trafiły Podróżówki zarejestrowane przez obdarowanych.
          </p>
        </motion.div>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">Kraje z zarejestrowanymi Podróżówkami</span>
          </div>
        </div>
        ...
        {!isLoading && countryData.length === 0 && (
          <div className="mt-8 text-center">
            <p className="text-muted-foreground">
              Brak jeszcze zarejestrowanych Podróżówek. Bądź pierwszy i zarejestruj swoją!
            </p>
          </div>
        )}
      </div>
